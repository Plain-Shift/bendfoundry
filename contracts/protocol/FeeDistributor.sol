// source: https://etherscan.io/address/0x2338d34337dd0811b684640de74717b73f7b8059#readProxyContract

// this is a proxied address
...
contract FeeDistributor is IFeeDistributor, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function _distribute() internal {
    ...
    if (amount > 0) {
      IERC20Upgradeable(token).safeTransferFrom(bendCollector, address(this), amount);
    }
  ...
  function _claimable(address _addr, uint256 _lastDistributeTime) internal view returns (Claimable memory) {
    ...
    IVeBend.Point memory userPoint = veBEND.getUserPointHistory(_addr, userEpoch);
    ...
    for (uint256 i = 0; i < 52; i++) {
      ...
      if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
        ...
        if (balanceOf > 0 && _veSupply > 0) {
          toDistribute += (balanceOf * tokensPerWeek[weekCursor]) / _veSupply;
        }
  ...
  function claim(bool weth) external override nonReentrant returns (uint256) {
    ...
    Claimable memory _st_claimable = _claimable(_sender, lastDistributeTime);
    ...
    if (amount != 0) {
      ...
      emit Claimed(_sender, amount, _st_claimable.userEpoch, _st_claimable.maxUserEpoch);
    }
